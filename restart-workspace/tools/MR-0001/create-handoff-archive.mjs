...
if (showHelp) {
  console.log(`Usage:
  node restart-workspace/tools/MR-0001/create-handoff-archive.mjs [--dry-run] [--allow-dirty]
...
const repoCheck = run("node", ["restart-workspace/tools/repo-check.mjs"]);
const repoCheckText = `${repoCheck.stdout}${repoCheck.stderr}`;
if (repoCheck.status !== 0) {
  console.error("ThreatForge local handoff archive generation refused.");
  console.error("restart-workspace repo-check failed.");
...
Language: Italian.
Delivery: concise, micro-steps, ZIP drop-ins, commands to apply/test/commit.
Governance rule: no code/tool without first defining requirement/ADR/GOV and linking it through registries and implementation trace.
