...
} from "./methodology-specific-analysis-record-model.mjs";

/**
 * @file Methodology-specific analysis record editor schema materializer.
 *
 ...
 * canonical methodology-specific analysis record model and representation
 ...
 * - neither mode modifies analysis records or governed source registries.
 */
...
export const methodologySpecificAnalysisRecordSchemaProjectPath =
  ".vscode/schemas/" +
  "methodology-specific-analysis-record.schema.json";
 ...
 */
export function formatMethodologySpecificAnalysisRecordSchema(value) {
  if (!isRecord(value)) {
    throw new TypeError(
      "Methodology-specific analysis record schema must be an object.",
    );
  }
...
function materializeField(field) {
  if (!isRecord(field)) {
    throw new TypeError(
      "Canonical analysis record profile field must be an object.",
    );
  }
 ...
 */
export function buildMethodologySpecificAnalysisRecordEditorSchema() {
  const profile = methodologySpecificAnalysisRecordProfile;
  const model = methodologySpecificAnalysisRecordModel;
  ...
  if (
    ...
    throw new Error(
      "Canonical analysis record model and profile are inconsistent.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Analysis record editor schema cannot materialize a governed document model.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Canonical analysis record profile is incomplete.",
    );
  }
  ...
  return {
    ...
    $id:
      ...
      "methodology-specific-analysis-record.schema.json",
    title: "Methodology-specific analysis record",
    description:
    ...
    "x-threatforge": {
      ...
      canonical_source:
        "tools/MR-0005/lib/" +
        "methodology-specific-analysis-record-model.mjs",
    },
  };
}
...
function readMaterializedSchema(absolutePath) {
  if (!fs.existsSync(absolutePath)) {
    throw new Error(
      "Materialized methodology-specific analysis record schema " +
      `is missing: ${methodologySpecificAnalysisRecordSchemaProjectPath}.`,
    );
  }
...
function writeAtomically(absolutePath, content) {
  ...
  try {
    ...
    throw new Error(
      "Cannot atomically materialize methodology-specific " +
      `analysis record schema: ${error.message}`,
    );
  }
}
 ...
 * }} Operation result.
 */
export function materializeMethodologySpecificAnalysisRecordSchema(
  input,
) {
  ...
  const generatedText =
    formatMethodologySpecificAnalysisRecordSchema(schema);
  ...
  if (mode === "check") {
    ...
    const existingText =
      formatMethodologySpecificAnalysisRecordSchema(existing);

    if (existingText !== generatedText) {
      throw new Error(
        "Materialized methodology-specific analysis record " +
        `schema is stale: ${methodologySpecificAnalysisRecordSchemaProjectPath}.`,
      );
    }
    ...
    return {
      mode,
      status: "current",
      path: methodologySpecificAnalysisRecordSchemaProjectPath,
      schema_id: schema.$id,
...
function main() {
  ...
  console.log(
    "Methodology-specific analysis record schema materialization succeeded.",
  );
...
if (import.meta.url === directExecutionUrl) {
  try {
    ...
    console.error(
      "Methodology-specific analysis record schema " +
