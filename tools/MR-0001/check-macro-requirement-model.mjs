...
/**
 * @file Macro-requirement complete-model checker.
 *
 ...
 *
 * Validates the active Macro-requirement corpus in blocking enforcement mode
 * and writes deterministic JSON and Markdown evidence reports.
 */

const scriptPath = fileURLToPath(import.meta.url);
const rootDir = process.env.TF_MACRO_REQUIREMENT_MODEL_ROOT
  ? path.resolve(process.env.TF_MACRO_REQUIREMENT_MODEL_ROOT)
...
let result;
try {
  result = validateMacroRequirementModel({ rootDir });
} catch (error) {
  console.error(
    `Macro-requirement model validation could not run: ${error.message}`,
  );
  process.exit(2);
}
...
const report = {
  checker: "check-macro-requirement-model",
  mode: "enforce",
...
fs.writeFileSync(
  path.join(reportDir, "macro-requirement.report.json"),
  `${JSON.stringify(report, null, 2)}\n`,
  "utf8",
);

const markdown = [
  "# Macro-requirement model report",
  "",
  `Mode: ${report.mode}`,
...
fs.writeFileSync(
  path.join(reportDir, "macro-requirement.report.md"),
  markdown.join("\n"),
  "utf8",
);
...
console.log(
  report.error_count === 0
    ? "Macro-requirement model check passed."
    : "Macro-requirement model check failed.",
);
...
console.log(
  "Report: artifacts/governed-document-models/macro-requirement.report.json",
);
