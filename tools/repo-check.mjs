...
/**
 * @file ThreatForge local check orchestrator.
 *
 * @implementsRequirement MR-0001ADR-0003REQ-0001GOV-0001
 * @derivedFromDecision MR-0001/ADR-0003
 * @macroRequirement MR-0001
 *
 * This local orchestrator reads the governed ThreatForge checks registry,
 * executes active checks in registry order, preserves checker output, and
 * summarizes the result for the canonical ThreatForge workspace.
 *
 * Side effects: reads the ThreatForge check registry, executes registered
...
const checksRegistryProjectPath =
  process.env.TF_LOCAL_GOVERNANCE_CHECKS_REGISTRY_PATH ??
...
/**
 * Parses the restricted YAML subset used by current ThreatForge registries.
 *
...
/**
 * Loads and validates local governance check records.
 *
...
function loadActiveChecks() {
  ...
  if (!fs.existsSync(registryPath)) {
    errors.push(`Local governance checks registry is missing: ${checksRegistryProjectPath}`);
    return [];
  }
  ...
  for (const check of registry.checks) {
    ...
    if (!id) {
      errors.push("Local governance check record is missing id.");
      continue;
    }

    if (ids.has(id)) errors.push(`Duplicate local governance check id: ${id}`);
    ids.add(id);
...
console.log("ThreatForge check started.");
...
if (failed) {
  console.error("ThreatForge check failed.");
  process.exit(1);
}

console.log("ThreatForge check passed.");
