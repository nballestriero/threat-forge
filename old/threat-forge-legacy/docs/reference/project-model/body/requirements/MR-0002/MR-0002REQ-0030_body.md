# MR-0002REQ-0030 — Governed entity collection view-model
...
The Project Model Explorer must define normalized read-only collection view-models for governed project-model entities.
...
- Collections must support at least macro requirements, requirements, ADR/decisions, governed documents where available, and taxonomy records.
- Collection records must include stable id, kind, label/title, status where applicable, summary metadata, and navigation target information.
- Collections may support query, filter, sorting and pagination parameters in future implementation.
- Collection records must not expose filesystem paths as the primary application identity.
