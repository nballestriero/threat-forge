...
A generated child project skeleton must include the minimal governed documentation structure, operating guides, standard Project Model registry/body/graph roots, local gate entrypoints, and lifecycle placeholders required to enforce documentation before implementation code.
...
Child-project documentation sources must be declared by reusing the same standard Project Model structure used by threat-forge itself: governed registries, governed body files, graph records, body-format profiles, and controlled taxonomy registries under `docs/reference/project-model/`. Threat-forge must not introduce a separate child-project document-source manifest format and must not discover arbitrary files as canonical governed documentation. Future loaders and validators must reject traversal, external roots, undeclared paths, and documentation outside the standard governed roots.
...
- defining standard Project Model document-source reuse and containment controls;
...
- implementing standard Project Model skeleton/source validators;
...
- Standard Project Model source controls reduce ambiguity between canonical governed documentation and free-form explanatory material.
