# MR-0002REQ-0034 — Capability-aware navigation view-model
...
The Governance Console must define a capability-aware navigation view-model so menus, routes and visible sections are decided by backend or policy-boundary outputs rather than hardcoded React role checks.
...
- The navigation view-model must include workspace type, visible navigation sections, required capabilities and disabled/hidden state where relevant.
- The bootstrap registered-user policy may return broad read-only capabilities for the first UI.
- The view-model must be compatible with future dynamic RBAC or configurable access policy.
- Frontend components must render navigation from this view-model rather than permanently hardcoding role checks or workspace-specific menu logic.
